...
SENDER_REPEATER_CHANNEL = 0x54
REPEATER_SENDER_CHANNEL = 0x5A
...
def ack_or_timeout(receiver):
    ...
    while (not receiver.available(pipes[0]) and (time.time() - timeout_starts) < 0.1):
        if receiver.available(pipes[0]):
            recv_buffer = []
            receiver.read(recv_buffer, receiver.getDynamicPayloadSize())
            ackPayload = bytes(recv_buffer)
            ...
            if (srcAddr == nextHopID) and (dstAddr == deviceID) and (ackNack == 3) and (pckSeqNum == count):
                return srcAddr, dstAddr, ackNack, pckSeqNum
        time.sleep(0.01)
...
def main_Source():
    ...
    sender = initialize_radios (0,16 , SENDER_REPEATER_CHANNEL)
    receiver = initialize_radios (1, 25, REPEATER_SENDER_CHANNEL)
...
def main_Repeater():
    ...
    #transmitData(sender, receiver, deviceID, nextHopID, int(2))
